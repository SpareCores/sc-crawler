...
def test_vultr_inventory_database_prices_uses_hourly_or_monthly():
    vendor = Mock(vendor_id="vultr")
    vendor.databases = [
    ...
    by_id = {row["database_id"]: row for row in rows}
    assert by_id["vultr-dbaas-cc-2-80-4"]["allocation"] == Allocation.ONDEMAND
    assert by_id["vultr-dbaas-cc-2-80-4"]["unit"] == PriceUnit.HOUR
    assert by_id["vultr-dbaas-cc-2-80-4"]["price"] == 0.09
    assert by_id["vultr-dbaas-cc-4-160-8-ha"]["unit"] == PriceUnit.MONTH
    assert by_id["vultr-dbaas-cc-4-160-8-ha"]["ha"] == DatabaseHaLevel.SINGLE_ZONE
