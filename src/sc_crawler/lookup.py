...
benchmarks: List[Benchmark] = [
    ...
    Benchmark(
        benchmark_id="hammerdb_postgres_multi:nopm",
        name="HammerDB Postgres OLTP (multi-VM)",
        category="Database",
        description="HammerDB TPROC-C on a remote client VM against Postgres on the DB host (multi-VM topology). Score is peak NOPM at adaptive peak concurrency. The 'async' durability tier (synchronous_commit=off, fsync on) is the comparable compute score; 'durable' (synchronous_commit=on) reflects disk-dependent durable-commit throughput.",
        framework="hammerdb_postgres_multi",
        measurement="nopm",
        config_fields={
            "topology": "Deployment topology (multi_vm).",
            "cache_tier": "Cache tier label (c100, c30).",
            "cache_ratio": "Working-set to buffer ratio.",
            "durability": "async = synchronous_commit=off (comparable compute score); durable = synchronous_commit=on (disk-dependent).",
            "peak_concurrency": "Virtual users at peak throughput.",
            "client_rtt_ms": "Client-to-DB round-trip latency (ms) on a warm SELECT 1 connection.",
            "latency_ms": "Workload transaction latency at peak concurrency (ms): p50, p95, p99, avg, min, max.",
            "workload": "HammerDB workload (e.g. tpcc).",
        },
        unit="New Orders Per Minute (NOPM)",
    ),
    Benchmark(
        benchmark_id="hammerdb_postgres_multi:qphh",
        name="HammerDB Postgres OLAP (multi-VM)",
        category="Database",
        description="HammerDB TPC-H on a remote client VM against Postgres on the DB host (multi-VM topology). Score is QphH at adaptive peak concurrency. The measured phase is read-only, so synchronous_commit has no effect on the score; durability is recorded for consistency.",
        framework="hammerdb_postgres_multi",
        measurement="qphh",
        config_fields={
            "topology": "Deployment topology (multi_vm).",
            "cache_tier": "Cache tier label (c100, c30).",
            "cache_ratio": "Working-set to buffer ratio.",
            "durability": "Postgres synchronous_commit tier (durable for OLAP; does not affect read-only TPC-H scoring).",
            "peak_concurrency": "Virtual users at peak throughput.",
            "client_rtt_ms": "Client-to-DB round-trip latency (ms) on a warm SELECT 1 connection.",
            "latency_ms": "Query latency at peak concurrency (ms) when reported by the workload driver.",
            "workload": "HammerDB workload (tpch).",
        },
        unit="Query-per-hour (QphH)",
    ),
    Benchmark(
        benchmark_id="benchbase_postgres_multi:tpm",
        name="BenchBase Postgres (multi-VM)",
        category="Database",
        description="BenchBase workloads (wikipedia, ycsb, …) on a remote client VM against Postgres on the DB host (multi-VM topology). Score is peak TPM at adaptive peak concurrency. Write-heavy workloads (ycsb) use the async durability tier (synchronous_commit=off, fsync on) for comparable compute scoring; read-heavy workloads (wikipedia) use durable.",
        framework="benchbase_postgres_multi",
        measurement="tpm",
        config_fields={
            "topology": "Deployment topology (multi_vm).",
            "cache_tier": "Cache tier label (c100, c30).",
            "cache_ratio": "Working-set to buffer ratio.",
            "durability": "async = synchronous_commit=off (write-heavy workloads); durable = synchronous_commit=on.",
            "peak_concurrency": "Terminals at peak throughput.",
            "client_rtt_ms": "Client-to-DB round-trip latency (ms) on a warm SELECT 1 connection.",
            "latency_ms": "Workload transaction latency at peak concurrency (ms): p50, p95, p99, avg, min, max.",
            "workload": "BenchBase benchmark name.",
        },
