...
benchmarks: List[Benchmark] = [
    ...
    Benchmark(
        ...
        name="pgbench Postgres (multi-VM)",
        category="Database",
        description="pgbench on a remote client VM against Postgres on the DB host (multi-VM topology). Score is peak TPM across the measured concurrency ladder plus adaptive upward search. The durable workload is read-only `pgbench -S`; the async workload is `tpcb-like` with `synchronous_commit=off` for comparable write-throughput scoring.",
        framework="pgbench_postgres_multi",
        measurement="tpm",
        config_fields={
            "topology": "Deployment topology (multi_vm).",
            "durability": "Postgres synchronous_commit tier: durable = on, async = off.",
            "scalefactor": "pgbench scale factor used for the measured size.",
            "peak_scalefactor": "Scale factor of the size that produced the headline peak score.",
            "peak_concurrency": "Clients at peak throughput.",
            "profile_max_clients": "Planned client-cap rung for the profile before adaptive extension.",
            "profile_hard_max_clients": "Hard upper cap for adaptive client search.",
            "latency_ms": "Workload latency at peak concurrency (ms): p50, p95, p99, avg, samples.",
            "workload": "pgbench workload (`pgbench_ro` or `pgbench_tpcb`).",
        },
