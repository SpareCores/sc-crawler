...
benchmarks: List[Benchmark] = [
    ...
    Benchmark(
        benchmark_id="membench:bandwidth",
        name="Memory bandwidth",
        description="Measures aggregate read, write, and copy bandwidth across all threads using OpenMP parallelization.",
        framework="membench",
        measurement="memory_bandwidth",
        config_fields={
            "size_kb": "Per-thread buffer size in KB (-s).",
            "tries": "Number of repeat runs, best result reported (-r).",
            "time_limit": "Maximum runtime in seconds (-t).",
            "operation": "Operation to run: read, write, copy, or latency (-o).",
            "huge_pages": "Enable huge pages for large buffers (-H).",
        },
        unit="MB/s",
    ),
    Benchmark(
