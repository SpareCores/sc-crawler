...
_RDS_PARALLEL_WORKERS = 32
_RDS_ORDERABLE_PAGE_SIZE = 1000
_RDS_REFERENCE_REGION = "us-east-1"


@cachier(separate_files=True)
def _describe_orderable_db_instance_options(region: str) -> list:
    rds = boto3.client("rds", region_name=region)
    paginator = rds.get_paginator("describe_orderable_db_instance_options")
    options = []
    for page in paginator.paginate(
        Engine="postgres",
        PaginationConfig={"PageSize": _RDS_ORDERABLE_PAGE_SIZE},
    ):
...
@cachier(separate_files=True)
def _describe_db_engine_versions(region: str) -> list[str]:
    rds = boto3.client("rds", region_name=region)
...
@cachier(separate_files=True)
def _reference_orderable_metadata(
    region: str = _RDS_REFERENCE_REGION,
) -> dict[str, dict]:
    """Collapse one reference-region orderable scan into class flags + storage bounds."""
    class_flags: dict[str, dict] = {}
    storage_bounds: dict[str, dict] = {}
    for opt in _describe_orderable_db_instance_options(region):
        database_id = opt.get("DBInstanceClass")
        if database_id:
            entry = class_flags.setdefault(
                database_id,
                {"ha_supported": False, "storage_autoscaling": False},
            )
            if opt.get("MultiAZCapable"):
                entry["ha_supported"] = True
            if opt.get("SupportsStorageAutoscaling"):
                entry["storage_autoscaling"] = True
    ...
    return {"class_flags": class_flags, "storage_bounds": storage_bounds}


@cachier(hash_func=jsoned_hash, separate_files=True)
def _boto_get_rds_products() -> list:
    """Fetch all PostgreSQL RDS products once; filter client-side like EC2 server prices."""
...
@cachier(separate_files=True)
def _ec2_instance_type_lookup() -> dict[str, dict]:
    return {
        spec["InstanceType"]: spec
        for spec in _boto_describe_instance_types("us-east-1")
    }


def _active_rds_regions(vendor: Vendor) -> list[str]:
    return [
        region.region_id
        for region in vendor.regions
        if region.status == Status.ACTIVE and region.region_id in _RDS_REGION_LOCATION
    ]


def _describe_db_engine_versions_with_progress(
    region: str, vendor: Vendor
) -> list[str]:
    versions = _describe_db_engine_versions(region)
    vendor.progress_tracker.advance_task()
    return versions


def _parallel_fetch_db_engine_versions(vendor: Vendor) -> dict[str, list[str]]:
    regions = _active_rds_regions(vendor)
    vendor.progress_tracker.start_task(
        name="Scanning region(s) for PostgreSQL engine version(s)",
        total=len(regions),
    )
    with ThreadPoolExecutor(max_workers=_RDS_PARALLEL_WORKERS) as executor:
        results = executor.map(
            _describe_db_engine_versions_with_progress,
            regions,
            repeat(vendor),
        )
    vendor.progress_tracker.hide_task()
...
def _rds_instance_products_by_region(
    products: list, regions: dict
) -> dict[str, dict[str, dict]]:
    by_region: dict[str, dict[str, dict]] = {}
    for product in products:
        attrs = product["product"]["attributes"]
        if attrs.get("productFamily") != "Database Instance":
            continue
        if attrs.get("deploymentOption", "Single-AZ") != "Single-AZ":
            continue
        try:
            region = regions[attrs["location"]]
        except KeyError:
            continue
        instance_type = attrs.get("instanceType")
        if instance_type:
            by_region.setdefault(region.region_id, {})[instance_type] = product
    return by_region


def _rds_hourly_price(product: dict) -> float | None:
    price, _currency = _extract_ondemand_price(product.get("terms", {}))
    return float(price) if price is not None else None


def _parse_memory_gib(value) -> float | None:
    if value is None:
        return None
    if isinstance(value, (int, float)):
        return float(value)
    match = re.search(r"(\d+(?:\.\d+)?)\s*G", str(value), re.IGNORECASE)
...
def _instance_compute(
    ...
    ec2_type = database_id[3:] if database_id.startswith("db.") else database_id
    spec = instance_specs.get(ec2_type)
    if spec:
        vcpus = spec.get("VCpuInfo", {}).get("DefaultVCpus")
        memory_mib = spec.get("MemoryInfo", {}).get("SizeInMiB")
        return vcpus, int(memory_mib) if memory_mib else None
    if product:
        attrs = product["product"]["attributes"]
        vcpus = attrs.get("vcpu")
        memory_gib = _parse_memory_gib(attrs.get("memory"))
        memory_mib = int(memory_gib * 1024) if memory_gib else None
        return int(float(vcpus)) if vcpus else None, memory_mib
    return None, None


def inventory_databases(vendor):
    """TODO: Not implemented properly yet; coming soon."""
    return []
    vendor.progress_tracker.start_task(
        name="Searching for database catalog price(s)", total=None
    )
    products = _boto_get_rds_products()
    vendor.progress_tracker.hide_task()

    regions = scmodels_to_dict(vendor.regions, keys=["name", "aliases"])
    prices_by_region = _rds_instance_products_by_region(products, regions)
    instance_specs = _ec2_instance_type_lookup()
    versions_by_region = _parallel_fetch_db_engine_versions(vendor)

    vendor.progress_tracker.start_task(
        name="Fetching orderable DB class metadata", total=1
    )
    class_flags = _reference_orderable_metadata()["class_flags"]
    vendor.progress_tracker.advance_task()
    vendor.progress_tracker.hide_task()

    rows = []
    active_regions = _active_rds_regions(vendor)
    vendor.progress_tracker.start_task(
        name="Preprocessing database(s)", total=len(active_regions)
    )
    for region_id in active_regions:
        engine_versions = versions_by_region.get(region_id, [])
        for database_id, product in prices_by_region.get(region_id, {}).items():
            flags = class_flags.get(database_id, {})
            vcpus, memory_amount = _instance_compute(
                database_id, product, instance_specs
            )
            rows.append(
                {
                    "vendor_id": vendor.vendor_id,
                    ...
                    "family": (
                        database_id.split(".")[1][:1] if "." in database_id else None
                    ),
                    "vcpus": vcpus,
                    ...
                    "storage_type": None,
                    "ha_supported": flags.get("ha_supported", False),
                    "storage_autoscaling": flags.get("storage_autoscaling", False),
...
def inventory_database_prices(vendor):
    ...
    for product in products:
        try:
            ...
            hourly = _rds_hourly_price(product)
            if not database_id or hourly is None:
                continue
            region = regions[attrs["location"]]
            items.append(
                {
...
def inventory_database_storages(vendor):
    ...
    return []
    vendor.progress_tracker.start_task(
        name="Fetching orderable DB storage metadata", total=1
    )
    storage_bounds = _reference_orderable_metadata()["storage_bounds"]
    vendor.progress_tracker.advance_task()
    vendor.progress_tracker.hide_task()
        ...
        items.append(
            {
                ...
                "name": storage_id,
                "description": f"RDS storage type {storage_id}",
...
def inventory_database_storage_prices(vendor):
    ...
    for product in products:
        try:
            attrs = product["product"]["attributes"]
            if attrs.get("productFamily") != "Database Storage":
                continue
            storage_id = (
                attrs.get("storageMedia") or attrs.get("volumeType") or ""
            ).lower()
            if not storage_id:
                storage_id = attrs.get("volumeApiName", "").lower()
            if not storage_id:
                continue
            hourly = _rds_hourly_price(product)
            if hourly is None:
