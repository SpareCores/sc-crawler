...
def get_regions():
    """
    Gathers the available regions in a list format. Uses the standard client object generated by the "get_client" function. Returns a list.
    """
    client = get_client()
...
def get_zones(tempcl):
    """
    Gathers the zones available in given region. Takes a client object as a parameter. Returns a list.
    """
    #client = get_client()
...
def get_instance_types(tempclient):
    """
    Gathers the available ECS instance types available in the given region. Receives a client object as a parameter. Returns a list.
    """
...
def get_disk_capabilities(tempclient):
    """
    Gathers information on pricing data on modules related to ECS instances. Returns a dictionary.
    """
...
def get_instance_price(tempclient, instance_type):
    """
    Gathers information on pricing data of ECS instances. Returns a dictionary.
    """
            ...
            for zone in zone_info_in_list.get("Zones").get("Zone"):
                ...
                for instancetype in instance_info_in_list.get("InstanceTypes").get("InstanceType"):
                    try:
                        ...
                        print("Adding instance info - Region: {tcl}, instance type: {itype}".format(tcl=tempclient.get_region_id(), itype=instancetype.get("InstanceTypeId")))
                        ...
                        print(se)
                        print("Failed to add instance info - Region: {tcl}, ZoneID: {zid}, instance type: {itype}".format(tcl=tempclient.get_region_id(), zid= zone.get("ZoneId"), itype=instancetype.get("InstanceTypeId")))
            ...
            if disk == "cloud_essd":
                for pl in options_essd_disk_performance_level:
                    ...
                    try:
                        ...
                        #print(se)
                        print("se.get_error_msg: "+ str(se.get_error_msg())) 
                        print("se.get_error_code: "+ str(se.get_error_code())) 
                        print("se.get_error_type: "+ str(se.get_error_type()))
                        try:
                            if "OperationDenied.PerformanceLevelNotMatch" in str(se.get_error_code()):
                                print("Error indicates that this zone is likely not supporting this disk type.")
                            elif "InvalidDataDiskCategory.ValueNotSupported" in str(se.get_error_code()):
                                print("Error indicates likely invalid region / disk combination.")
                        except ServerException as se:
                            print("Second error within the same cycle. " + str(se))
                    # except ClientException as ce:
                    #     print("ce.get_error_msg: "+ str(ce.get_error_msg())) 
                    #     print("ce.get_error_code: "+ str(ce.get_error_code())) 
                    #     print("ce.get_error_type: "+ str(ce.get_error_type()))
                ...
                try:
                    ...
                    print(se)
                    print("tempclient: {tcl}, disk: {disk}, size: {size}".format(tcl=tempclient.get_region_id(), disk=disk, size=2000))
